...
Texture *SetRenderTextureRS(RenderState *rs, char *texName, int mode) {
	...
	tex = SceneFindTexture(texName);
	if (tex && GetTypeTexture(tex) != mode) {
		fprintf(stderr, "[E] Texture \"%s\" already exists and has incompatible type. Destroy it first and then create another one.\n", texName);
		exit(1);
	}
	if (!tex) {
		// Init texture (no mipmap)
		tex = SceneRegisterVoidTexture(texName);
		// Set standard w/h
		h = w = 512;
	}

	if (mode == AGA_TEX_DEPTH) {
		SetTextureDepthMap(tex, h, w);
	} else {
		// mode == AGA_TEX_COLOR
		SetTextureColorMap(tex, h, w);
	}
