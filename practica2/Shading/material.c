...
void DestroyMaterial( Material **theMaterial ) {
	...
	if (thisMaterial->bumpmap)   DestroyTexture(&(thisMaterial->bumpmap));
...
int HasTextureMaterial(Material *thisMaterial) {
	return thisMaterial->texture != NULL;
}

int HasBumpMaterial(Material *thisMaterial) {
	return thisMaterial->bumpmap != NULL;
}
...
void PrintMaterial( Material *thisMaterial ) {
	...
	printf("\n");
	if (HasTextureMaterial(thisMaterial)) {
		printf("Texture: ");
		PrintTexture(thisMaterial->texture);
	}
	if (HasBumpMaterial(thisMaterial)) {
		printf("Bump map: ");
		PrintTexture(thisMaterial->bumpmap);
	}
}
