...
void Trfm3DTransformPoint( const trfm3D *thisTrfm3D, float *xTr, float *yTr, float *zTr ) {
	...
	//Px' = Px*r1X*scale + Py*r2X*scale + Pz*r3X*scale + trX
	*xTr = tmpX * thisTrfm3D->r1X * scale + tmpY * thisTrfm3D->r2X * scale + tmpZ * thisTrfm3D->r3X * scale + thisTrfm3D->trX;

	//Py' = Px*r1Y*scale + Py*r2Y*scale + Pz*r3Y*scale + trY
	*yTr = tmpX * thisTrfm3D->r1Y * scale + tmpY * thisTrfm3D->r2Y * scale + tmpZ * thisTrfm3D->r3Y * scale + thisTrfm3D->trY;

	//Pz' = Px*r1Z*scale + Py*r2Z*scale + Pz*r3Z*scale + trZ
	*zTr = tmpX * thisTrfm3D->r1Z * scale + tmpY * thisTrfm3D->r2Z * scale + tmpZ * thisTrfm3D->r3Z * scale + thisTrfm3D->trZ;
}
...
void TransformVectorTrfm3D( const trfm3D *thisTrfm3D, float *xTr, float *yTr, float *zTr ) {
	...
	//Vx' = Px*r1X*scale + Py*r2X*scale + Pz*r3X*scale
	*xTr = tmpX * thisTrfm3D->r1X * scale + tmpY * thisTrfm3D->r2X * scale + tmpZ * thisTrfm3D->r3X * scale;

	//Vy' = Px*r1Y*scale + Py*r2Y*scale + Pz*r3Y*scale
	*yTr = tmpX * thisTrfm3D->r1Y * scale + tmpY * thisTrfm3D->r2Y * scale + tmpZ * thisTrfm3D->r3Y * scale;

	//Vz' = Px*r1Z*scale + Py*r2Z*scale + Pz*r3Z*scale
	*zTr = tmpX * thisTrfm3D->r1Z * scale + tmpY * thisTrfm3D->r2Z * scale + tmpZ * thisTrfm3D->r3Z * scale;
}
...
void SetRotAxisTrfm3D(trfm3D *thisTrfm, float vectorX, float vectorY, float vectorZ,
	...
	SetTransTrfm3D(thisTrfm, pointX, pointY, pointZ );
	AddRotVecTrfm3D(thisTrfm, vectorX, vectorY, vectorZ, angle);
	AddTransTrfm3D(thisTrfm,  -pointX,  -pointY,  -pointZ );
}
...
void AddRotAxisTrfm3D( trfm3D *thisTrfm3D,
	...
	SetRotAxisTrfm3D(&localT, vectorX, vectorY, vectorZ,
					 pointX, pointY, pointZ, angle);
	CompositionTrfm3D( thisTrfm3D, &localT );
}
